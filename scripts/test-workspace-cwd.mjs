...
//   · alpha 线(≥ 0.1.6-alpha.2,含最新 0.1.7-alpha.1):`@deepseek-ai/dsh-api-session-controller/lib/client.js` 里 list.set({ids,byId,phase,subagentsByParent,jobsBySession})
//     —— 全文件 0 处 `current:`;`ui-deliverables` 的 ReviewTab 用 `useSessions(s => s.byId[sessionId]?.cwd)`;
//   · rc 线(≤ 0.1.5-rc.3,含最新 rc):`.../sessions/service.ts` 的 SessionListState 里仍有 `current: SessionId | undefined`。
//
...
/** rc 线(DSH ≤ 0.1.5-rc.3)的会话列表快照:带 `current`。 */
function sessionsRcLine({ current, rows }) {
  const byId = {};
  for (const row of rows) byId[row.id] = row;
  return { ids: rows.map((r) => r.id), byId, current, phase: 'ready' };
}

/** alpha 线(DSH ≥ 0.1.6-alpha.2)的会话列表快照:**没有** `current`(上游把它移出了列表 store)。 */
function sessionsLatestLine({ rows }) {
  const byId = {};
  for (const row of rows) byId[row.id] = row;
...
await test('最新线(≥0.1.6-alpha.2):会话标准 prop 的 sessionId → 该会话的 cwd(修复点)', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'b', cwd: WS_BETA }] });
  assert.equal(pickWorkspaceCwd({ sessions, workspaces: workspaces([]), sessionId: 'b' }), WS_BETA);
});

await test('最新线(≥0.1.6-alpha.2):同形状下读旧字段必然拿不到 —— 这就是 0.3.46 的空工作区根因', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'b', cwd: WS_BETA }] });
...
await test('rc 线(≤0.1.5-rc.3):列表快照上的 current 仍然认(向后兼容)', async () => {
  const sessions = sessionsRcLine({ current: 'a', rows: [{ id: 'a', cwd: WS_ALPHA }] });
...
await test('sessionId 优先于 current(切标签/看别的会话的文件时不能串工作区)', async () => {
  const sessions = sessionsRcLine({ current: 'a', rows: [{ id: 'a', cwd: WS_ALPHA }, { id: 'b', cwd: WS_BETA }] });
  assert.equal(pickWorkspaceCwd({ sessions, workspaces: workspaces([]), sessionId: 'b' }), WS_BETA);
});

await test('会话摘要没有 cwd 时退到工作区表(按 sessionIds 归属)', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'b' }] });
...
await test('文件 tab 的会话来自地址:地址里的 sessionId 一样能用', async () => {
  ...
  const sessions = sessionsLatestLine({ rows: [{ id: 'other', cwd: 'D:/repo/other' }] });
...
await test('根作用域(后台预热,无 sessionId)退到"最近活跃会话所属工作区"', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'recent', cwd: '' }, { id: 'old' }] });
  const ws = workspaces([
...
await test('根作用域:最近活跃会话不属于任何工作区时退到第一个工作区', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'loose', cwd: '' }] });
...
await test('拿不到任何信源 → undefined(绝不猜目录;调用方不传 cwd)', async () => {
  ...
  assert.equal(pickWorkspaceCwd({ sessions: sessionsLatestLine({ rows: [] }), workspaces: workspaces([]) }), undefined);
...
await test('路径原样返回(Windows 反斜杠不在本层做 URL 归一)', async () => {
  const sessions = sessionsLatestLine({ rows: [{ id: 'w', cwd: 'C:\\Users\\u\\proj' }] });
