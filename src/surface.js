 ...
 *
 * 另一个实测坑(0.2.2 修):跨源 iframe 离屏停放较久后被移回,元素命中/尺寸/可见性全对,
 * 但**画面不再重绘**(面板一片白,切换标签几次也不恢复)。实测 `translateZ(0)`、`opacity`
 * 微调都无效,`display:none → 强制重排 → 还原` 能在**同一个 JS 任务内**唤醒重绘:
 * 无可见闪烁,iframe 文档不重载、内部状态不变。故每次"从停放到停靠"都补一次 `nudgeRepaint()`。
 *
