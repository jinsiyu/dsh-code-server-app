...
// settings:**设置的数据域**。两条线都有这个服务(所以它可以是硬依赖),但**模型不同**:
//   · 旧线(rc 0.1.5-rc.x;也含"座位已搬到插件页、数据面还是旧的"0.1.6-alpha.2):
//     `settings.register(ns, schema)` → `scope.get()/watch()`,值持久化在官方 settings 文档;
//   · 新线(alpha ≥ 0.1.7-alpha.1):`register/get/watch` 已删除,配置就是**本插件条目自己的 Config**
//     (可编辑字段必须 `.volatile()`),读值走 `config.<field>.get()` 活引用,变更通知走
//     `settings/document-updated`。
// 两条线的选择在 apply 里**按能力探测**(`typeof settings.register === 'function'`),不比较版本号。
export const inject = ['connection', 'settings'];

/** profile 条目 id(cordis.patch.yml 的 `- id: code-server`)。
 *  新线里它同时也是**设置命名空间**:`ctx.configForms.get(entryId)` 与宿主的 settings.describe
 *  都按条目 id 取用(表单只展示"活动且可唯一定位"的条目)。客户端半部的 ENTRY_ID 必须与此一致。 */
export const SETTINGS_NS = 'code-server';

/**
 * 可配置字段表(**唯一真源**):两条数据线共用同一份字段定义,只在"是否 volatile"上分叉。
 *
 * 为什么必须分叉(0.3.66 实测):
 *   · 新线(alpha ≥ 0.1.7-alpha.1)把插件配置模型换成"插件条目自己的 Config":
 *     `ctx.settings.register(ns, schema)` 已删除;宿主只把 **`.volatile()` 字段**当作该条目的可编辑表单
 *     (settings.describe 按 volatile 投影并明确报错 `Config field "x" is not volatile`),
 *     且保存会**就地重解析**这个叶子(不重挂插件)⇒ 字段必须 volatile,读值必须走 config.<field>.get()。
 *   · 旧线要把同一份 schema 注册进官方 settings 域(`settings.register(SETTINGS_NS, schema)`),
 *     而**那里不能用带 volatile 的 schema**:schemastery 3.18.3 起 volatile 是**解析期**行为
 *     (`createVolatile(value)` 在 schema 里完成),旧的 settings 域不认识它,会把活引用直接交给线路
 *     (JSON 化后是空对象 `{}`)⇒ 卡片读到的全是空值。所以旧线注册 `SettingsSchema`(同字段、无 volatile)。
 *   · rc 线的 schemastery 是 3.18.2,**根本没有 `.volatile()`**(3.18.3 才加)⇒ 无条件调用会在模块
 *     求值期抛 `TypeError`、整棵插件树加载失败。故一律经 `vol()` 探测能力后再包。
 *
...
/** volatile 包装(**能力探测**,不看版本号):rc 线的 schemastery 3.18.2 没有这个方法。 */
const vol = (schema) => (typeof schema.volatile === 'function' ? schema.volatile() : schema);

/** 用同一份字段表建一条 schema;`wrap` 决定字段是否 volatile。 */
function settingShape(wrap) {
  const shape = {};
  for (const [key, make] of Object.entries(SETTING_FIELDS)) shape[key] = wrap(make());
  return shape;
}

/** **本插件条目的 Config**(新线:alpha ≥ 0.1.7-alpha.1 的配置模型)。
 *  字段全部 volatile:这是"宿主把该条目暴露成可编辑表单 + 保存就地重解析"的前提。 */
export const Config = z.object(settingShape(vol));

/** **旧线注册用 schema**(rc 0.1.5-rc.x 与 0.1.6-alpha.2 的 `settings.register`):
 *  同字段、**不带 volatile** —— 原因见 SETTING_FIELDS 上方那段(schemastery 的 volatile 是解析期行为,
 *  旧的 settings 域会把活引用交给线路,卡片会读到空对象)。 */
export const SettingsSchema = z.object(settingShape((schema) => schema));
...
export async function apply(ctx, config) {
  ...
  // ---- 设置:两条数据线(见文件头与 SETTING_FIELDS 的说明) ----
  // 判据是**能力**不是版本号:旧线的 settings 服务有 register(rc 0.1.5-rc.x,以及数据面还没换的
  // 0.1.6-alpha.2);新线(alpha ≥ 0.1.7-alpha.1)删掉了它,配置就是本条目自己的 volatile Config。
  const settingsSvc = ctx.get('settings');
  const legacySettings = settingsSvc !== undefined && settingsSvc !== null
    && typeof settingsSvc.register === 'function';

  /**
   * 读一个配置字段的**当前值**。
   *
   * 新线:宿主编译期把 volatile 叶子换成**活引用**(`{ get() }`,见 cosmokit 的 createVolatile),
   * 保存(表单 mutate → profile patch → loader 重解析)只就地更新这个叶子、**不重挂插件**,
   * 所以每次都现读引用才是"实时值"。旧线(以及 rc 线 3.18.2 的 schemastery,它没有 volatile)
   * 上 config.<field> 就是普通值 —— 同一个读法两条线都对。
   */
  const cfgValue = (field) => {
    const leaf = config === null || config === undefined ? undefined : config[field];
    if (leaf !== null && typeof leaf === 'object' && typeof leaf.get === 'function') return leaf.get();
    return leaf;
  };
  /** 有效值 = volatile 活叶子 / 行配置 config > 代码内默认(DEFAULT_CONFIG)。 */
  const readCfg = (field) => {
    const live = cfgValue(field);
    return live === undefined ? cfg[field] : live;
  };
  /**
   * 从**当前配置**拼出与旧线 `scope.get()` 同形的一份快照,交给同一个 commit 函数。
   * 默认值就地取 DEFAULT_CONFIG / normalize,与旧线 register 分支里的兜底逐项对齐。
   */
  const readSettingsSnapshot = () => ({
    serve: readCfg('serve') === 'dsh' ? 'dsh' : 'loopback',
    keepResident: typeof readCfg('keepResident') === 'boolean' ? readCfg('keepResident') : true,
    claimExtensions: typeof readCfg('claimExtensions') === 'string'
      ? normalizeClaimExtensions(readCfg('claimExtensions'))
      : DEFAULT_CLAIM_EXTENSIONS,
    fullscreenOnOpen: typeof readCfg('fullscreenOnOpen') === 'boolean' ? readCfg('fullscreenOnOpen') : true,
    editorBridge: readCfg('editorBridge') !== false,
    fim: readCfg('fim') === true,
    fimDebounceMs: clampDebounce(readCfg('fimDebounceMs')),
    fimMultiline: readCfg('fimMultiline') !== false,
    fimDisableGlobs: typeof readCfg('fimDisableGlobs') === 'string' ? readCfg('fimDisableGlobs') : '',
  });
  ...
  /** 编辑器桥开关(0.3.0):行配置 cfg.editorBridge 是种子,设置里可改并实时生效。 */
  let bridgeSetting = readCfg('editorBridge') !== false;
  /** FIM 补全开关(实验性,0.3.61):行配置 cfg.fim 是种子,设置里可改并实时生效。默认关。 */
  let fimSetting = readCfg('fim') === true;
  /** FIM 的三个子项(0.3.62):停顿毫秒数 / 是否多行 / 按 glob 禁用(原始文本,消费时解析)。 */
  let fimDebounceMs = clampDebounce(readCfg('fimDebounceMs'));
  let fimMultiline = readCfg('fimMultiline') !== false;
  let fimDisableGlobs = typeof readCfg('fimDisableGlobs') === 'string' ? readCfg('fimDisableGlobs') : '';
  /**
   * 把一份设置快照落到本地状态上,并在**值真的变了**时执行随动副作用。
   *
   * 两条线的触发点不同,但落到这里是同一段代码:
   *   · 旧线:`scope.watch(commitSettings)`(首次装配另走 initial=true,不触发副作用);
   *   · 新线:`ctx.on('settings/document-updated', …)` → readSettingsSnapshot() → 这里。
   * 副作用的清单(与 0.3.0 起的语义逐条一致):编辑器桥上下线、FIM 适配器注册/注销、
   * FIM 三个子项即时生效、状态里的四元组(serve/keepResident/claimExtensions/fullscreenOnOpen)。
   */
  const commitSettings = (next, opts) => {
    const initial = opts !== undefined && opts.initial === true;
    ...
    if (next != null && typeof next.editorBridge === 'boolean') {
      if (next.editorBridge !== bridgeSetting) {
        ...
        // 首次装配(initial)不做这件事:桥的启动由末尾那个 ctx.effect 负责,重复做会白起一次监听口。
        if (initial !== true) {
    ...
    if (next != null && typeof next.fim === 'boolean') {
      if (next.fim !== fimSetting) {
        ...
        if (initial !== true) void ensureFimAdapter();
      }
    }
    ...
    if (next != null && typeof next.keepResident === 'boolean') {
      if (next.keepResident !== keepResident) console.log(`[code-server] keepResident updated: ${next.keepResident}`);
  ...
  if (legacySettings) {
    // ---- 旧线:settings 命名空间(rc 0.1.5-rc.x;也含 0.1.6-alpha.2) ----
    try {
      // 注册**不带 volatile** 的 SettingsSchema:旧的 settings 域会把 volatile 活引用直接交给线路。
      ...
      if (rawDoc !== undefined && rawDoc !== null) {
        commitSettings(scope.get(), { initial: true });
      }
      scope.watch((next) => { commitSettings(next); });
    } catch (error) {
    ...
    // ---- 新线:本条目自己的 volatile Config(alpha ≥ 0.1.7-alpha.1) ----
    // 先按活叶子装一遍初值(不触发副作用),再把后续变更挂到 settings 的文档事件上:
    // volatile 叶子的就地重解析不重挂插件,所以随动必须由事件驱动。
    commitSettings(readSettingsSnapshot(), { initial: true });
    ctx.on('settings/document-updated', () => { commitSettings(readSettingsSnapshot()); });
  }

  // ---- 自带配置页的声明(0.3.66) ----
  // 本插件的设置面是客户端半部自绘的表单(插件页的 `plugins.bundle.config`),不是按 schema 自动生成的页面。
  // 上游要求:这类插件在 apply 中于**可选的** `ctx.inject(['settings'], …)` 子级内以 effect 注册
  // `configure({ auto: false }, ctx.fiber)` —— 子级指明策略所属的插件 fiber(迟加载/被替换的 Settings
  // 服务也会采用该策略),而"业务插件无需 Settings 即可运行"这条性质靠可选的 inject 保住。
  // 策略只影响"要不要自动生成页面",不移除配置读写(表单/描述/mutate 仍照常)。
  if (typeof ctx.inject === 'function') {
    try {
      ctx.inject(['settings'], (child) => {
        const svc = typeof child.get === 'function' ? child.get('settings') : child.settings;
        if (svc === undefined || svc === null || typeof svc.configure !== 'function') return; // 旧线没有这个能力
        child.effect(() => {
          try {
            return svc.configure({ auto: false }, ctx.fiber);
          } catch (error) {
            // 同一个 fiber 注册两次会抛(Settings presentation is already configured for this plugin
            // instance)。热重载下 apply 可能重跑,而"没有自动页面"只是外观问题 ⇒ 记一条日志就够。
            console.warn(`[code-server] settings presentation 未注册(不影响配置读写):${error && error.message ? error.message : error}`);
            return () => {};
          }
        }, 'code-server: settings presentation (auto: false)');
      });
    } catch (error) {
      console.warn(`[code-server] settings presentation 未注册(不影响配置读写):${error && error.message ? error.message : error}`);
    }
  ...
  function requestedServe() {
    const want = serveSetting === 'dsh' || readCfg('serve') === 'dsh' ? 'dsh' : 'loopback';
