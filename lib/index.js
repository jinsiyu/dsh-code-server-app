...
export async function apply(ctx, config) {
  ...
  async function handleBridgeSync(request) {
    ...
    const snapshot = bridgeThread.supported()
      ...
      : { available: false, sessionId: null, cursor: null, rev: 0, entries: [], error: '宿主没有 sessionController.follow(DSH 版本过旧)' };
    // **只对"懂修订号"的扩展省流量**(0.3.28):旧扩展(≤0.2.7)不知道 threadRev,一旦收到省略了
    // thread 的响应就会以为"宿主没有对话流能力"(面板真的会这么报错)。所以对旧扩展照旧发整份快照。
    const peerUnderstandsRev = Number.isSafeInteger(body.threadRev);
    const threadChanged = threadRev !== bridgeThreadRevSent || snapshot.available === false || !peerUnderstandsRev;
