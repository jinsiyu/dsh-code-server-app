...
/** 本插件的 package.json 版本(用于 ?v= 缓存击穿标记;读不到就退回 'dev')。 */
...
export async function apply(ctx, config) {
  ...
  async function startInner(cwdArg) {
    ...
    // ?v= 缓存击穿标记:**按"插件版本 + VS Code 树"生成**,不随激活变化 ——
    // 只在真正换了版本时失效缓存,平时正常命中(workbench.js 有 18MB,每次重启都重拉不划算)。
    env.DSHCS_HTML_TAG = `${pluginVersion()}-${state.env?.productPath ?? productPath() ?? 'dev'}`.replace(/[^A-Za-z0-9._-]/g, '');
