...
/** 插件自带 code-server 入口探测(优先级):
 *   1. node_modules 依赖安装(node_modules/code-server/out/node/entry.js,
 *      `npm install` 随插件安装,package.json dependencies 声明);
 *   2. 旧 runtime 目录(runtime/node_modules/code-server/...,
 *      README 早前的"runtime 恢复命令"安装方式,保留兼容)。
 * 均不存在 → 回退配置/PATH。 */
function bundledRuntimeEntry() {
  try {
    ...
    const candidates = [
      ...
      path.join(here, '..', 'runtime', 'node_modules', 'code-server', 'out', 'node', 'entry.js'),
    ];
...
function resolveLaunch(config) {
  ...
  // 自带 runtime 优先,除非用户显式配置了非默认 bin
