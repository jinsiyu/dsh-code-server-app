...
//
// 回退走 lib/dsh-resolve.mjs 的 dshRequire() —— 位置表就一份,%APPDATA%\npm\node_modules\@deepseek-ai\dsh
// **和** $DSH_HOME/profiles/node_modules\@deepseek-ai\dsh。只查前者是不够的(0.3.47 的安装冒烟实测):
// 只要 CLI 不是「普通用户 npm 全局装」,前者就不存在 ——
//   · CI / nvm / 工具缓存:npm 的全局前缀是 node 安装目录,不是 %APPDATA%\npm;
//   · pnpm global:装到 pnpm 自己的目录;
//   · 而后者由 DSH 每次 boot 自己 heal 出来(profile 层 junction),生产里一定在。
// 两条都拿不到时才抛错(不是静默降级:设置 schema 建不出来,插件没有意义)。
let z = null;
try {
...
export async function apply(ctx, config) {
   ...
   *   - `ids`:编辑器扩展面板声明要看的会话(每趟 /sync 上报);
   *   - `dialogIds`:DSH 页面里的对话框(0.3.24)声明要看的会话;
   *   - `at`:最近一次声明的时间(超过 5s 视为没人看)。
   */
  const bridgeWatch = { ids: [], dialogIds: [], at: 0 };
  /** 上次回给扩展的对话流修订号(0.3.27:没变化就不重传整份快照)。 */
  let bridgeThreadRevSent = -1;
  ...
  /** 客户端(self-declared)是否能把授权卡片画出来(0.3.30)。 */
  let bridgeApprovalsUi = false;
  let bridgeApprovalsUiLogged = false;
  ...
  function watchedSessions() {
    const all = [...bridgeWatch.ids, ...bridgeWatch.dialogIds];
    return [...new Set(all.filter((id) => typeof id === 'string' && id !== ''))].slice(0, 4);
  }
  /** 现在有没有人在看(授权拦截的唯一判据)。 */
  function hasWatcher() {
    const sessions = watchedSessions();
    if (sessions.length === 0) return false;
    // **对话框是宿主侧状态**(只在 /ask/close 时消失):不拿轮询新鲜度去判它 ——
    // 浏览器把后台标签的定时器节流到分钟级时,900ms 的心跳一断、5 秒窗口就过期,
    // 我们会误判"没人看"而放弃拦截(用户实测:授权卡片因此跑到 DSH 界面)。
    if (bridgeWatch.dialogIds.length > 0) return true;
    // 编辑器面板靠 /sync 的 600ms 心跳;窗口放宽到 60 秒(正常心跳远小于它)。
    return Date.now() - bridgeWatch.at < 60000;
  }

  const bridgeApproval = createApprovalInterceptor({
    board: bridgeApprovalBoard,
    holdMs: DEFAULT_HOLD_MS,
    // **只在客户端能画卡片时才抢答**(0.3.30):面板/对话框在看 **且** 它自己声明 approvalsUi。
    // 反例(用户实测):面板处于"宿主没有对话流能力"的坏状态时我们仍然抢答 ⇒ 官方卡片不出现、
    // 面板也不画 ⇒ 用户看到的是"没弹出授权",请求在宿主里干等 5 分钟。
    // 客户端能画卡片 = 编辑器面板自报可以的 **或** 对话框开着且有会话(0.3.33)。
    // "活着"由 /ask/state 轮询证明)。0.3.31 的日志给出了反例:用户从**对话框**提问 ⇒
    // 编辑器面板是 null ⇒ 旧判据误判成"画不出卡片",授权被让给了 DSH 界面。
    hasPanel: () => {
      const watcher = hasWatcher();
      const dialogCapable = askDialogLive();
      const verdict = watcher && (bridgeApprovalsUi || dialogCapable);
      // 判据的**原始值**一起落日志(0.3.35):只记结论的话,出问题还是只能猜。
      const line = 'hasPanel -> ' + verdict + ' (hasWatcher=' + watcher + ' approvalsUi=' + bridgeApprovalsUi
   ...
   *
   * 对话渲染从编辑器的 webview 面板搬到了 **DSH 页面里的浮动对话框**(client 半部,见 src/factory.js):
   * 面板是编辑器里的一个 tab/一列,而用户要的是"像 DSH 那样的对话框"。host 这边只负责:
   *   - 打开/关闭状态(编辑器扩展右键 → 桥 `/event` 上报 `ask-open`);
   *   - 状态与动作的 HTTP 面(`/api/code-server/ask/*`,与 DSH 同源、吃它的 cookie/Origin 校验);
   *   - 把面板产物(thread.js / thread.css)交给对话框注入(Lib 版本与 DSH 界面同版本,见 build-webview.mjs)。
   */
   ...
   * 官方卡片,用户在编辑器里什么都看不到。现在只有**真的收到过 `/ask/state` 轮询**(5 秒内)才算有
   * 这个能力;否则扩展退回编辑器面板 —— 那是验证过、能看能点的 UI。
   */
  ...
  /** 面板产物(thread.js / thread.css):对话框把它们注入 DSH 页面,所以由 host 读盘给文本。 */
  let askBundleCache = null;
  function askBundle() {
    if (askBundleCache !== null) return askBundleCache;
    const dir = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'assets', 'extensions', 'dshcs-editor-bridge', 'webview');
    const read = (name) => {
      try {
    ...
    // 产物缺失(打包漏了 build:webview)→ 明确报出来,不返回半个面板。
    if (js === null || css === null) {
      console.warn(`[code-server] 「问 DSH」对话框的产物缺失:${dir}(请在源码里跑 pnpm run build:webview)`);
      return null;
    }
    askBundleCache = { js, css };
    return askBundleCache;
  }
   ...
   * 本部署**界面**的版本(0.3.22)。
   *
   * 面板里的 markdown 渲染器是从 `@deepseek-ai/dsh-client-ui-primitives` 打包进去的,而那份
   * 渲染器就装在 `@deepseek-ai/dsh-web-frontend` 里(DSH Web 的前端包)。面板拿这个版本和
   * 自己打包时的版本比一比,不一致就在面板顶部显示一行提示 —— 版本错配的排版差异不许静默。
   */
  function uiVersion() {
    try {
      const entry = dshEntry();
      ...
      const candidates = [
        path.join(dir, 'node_modules', '@deepseek-ai', 'dsh-web-frontend', 'package.json'),
        path.join(dir, '..', 'dsh-web-frontend', 'package.json'),
      ];
   ...
   *  `/approve` 是唯一的非只读路由(只回答既有授权请求,见 handleBridgeApprove)。 */
  ...
  // 这里只登记卸载钩子:面板关掉 / IDE 停掉时把订阅与 pending 授权一起收干净。
  bridgeAnswerDispose = () => {
  ...
  async function handleBridgeSync(request) {
    ...
    bridgeWatch.ids = watch.slice(0, 4);
    bridgeApprovalsUi = body.approvalsUi === true;
    if (bridgeApprovalsUi !== bridgeApprovalsUiLogged) {
      bridgeApprovalsUiLogged = bridgeApprovalsUi;
      bridgeDiag('approvalsUi → ' + bridgeApprovalsUi + '(客户端可否显示授权卡片)');
    }
    bridgeWatch.at = Date.now();
    ...
    if (watchingNow !== bridgeWatchLogged || bridgeWatchWroteFirst !== true) {
      ...
      bridgeDiag('hasWatcher → ' + watchingNow
        + ' (面板=[' + bridgeWatch.ids.join(',') + '] 对话框=[' + bridgeWatch.dialogIds.join(',') + ']'
        + ' approvalsUi=' + bridgeApprovalsUi + ')');
    }
    ...
    if (askDialog.open && !askDialogLive() && askDialog.warned !== true) {
      ...
      console.warn('[code-server] 请求了「问 DSH」对话框,但客户端半部没有轮询 /ask/state:'
        ...
        + '此期间提问会退回编辑器面板,授权也在面板里批。');
    }
    // **对话流只在变化时回传**(0.3.27):以前每趟 /sync 都塞整份快照(≤120 条 × ≤8KB,可达 ~1MB / 600ms),
    // 实测把 /sync 拖成 3 秒超时 —— 扩展收不到响应 ⇒ 面板看不到 approvals ⇒ 授权卡片永远不出现
    // (卡片在宿主里干等到窗口结束才交给 DSH 界面)。现在给修订号,没变化就不重传。
    const threadRev = bridgeThread.rev(watchedSessions());
    const snapshot = bridgeThread.supported()
      ? bridgeThread.snapshot(watchedSessions())
      : { available: false, sessionId: null, cursor: null, rev: 0, entries: [], error: '宿主没有 sessionController.follow(DSH 版本过旧)' };
    // **按"客户端已经拿到哪一版"决定发不发**(0.3.29 修):0.3.28 比的是宿主自己上次发过的版本,
    // 于是"面板是后开的"这种情形下宿主认为没变化、什么都不发,面板手里一份快照都没有 ⇒ 旧模型把它
    // 当成"宿主没有对话流能力"(用户实测到的那条误报)。现在由客户端用 threadRev 声明自己持有的版本;
    // 不带这个字段的旧扩展一律收整份(不省流量,但绝不误报)。
    const peerRev = Number.isSafeInteger(body.threadRev) ? body.threadRev : null;
    const threadChanged = peerRev === null || peerRev !== threadRev || snapshot.available === false;
    bridgeThreadRevSent = threadRev;
    // 对话流走独立字段:每个会话给有界条目(见 lib/bridge-thread.mjs),
    // 授权待决列表也一并回去(面板显示卡片)。
    return jsonResponse({
      ok: true,
      events,
      ...
      approvals: bridgeApprovalBoard.snapshot(),
      // 授权卡片要在面板上倒计时,窗口长度由 host 给(面板不猜)。
      approvalHoldMs: DEFAULT_HOLD_MS,
      dshVersion: dshVersion(),
      // 界面版本:面板据此提示"内置渲染器与当前 DSH 界面版本不一致"。
      uiVersion: uiVersion(),
      // 能力探测(0.3.24;0.3.25 起以"客户端半部真的在轮询"为准):
      // 只有对话框活着的时候才让扩展走 ask-open;否则扩展开编辑器面板(那里也能批授权)。
  ...
  async function handleBridgeAsk(request) {
    ...
    // 会话流 + 授权拦截都在这里建立(只针对**被问过的**那个会话,agent 作用域):
    //   - thread:面板像 DSH 对话那样显示"新内容"(见 lib/bridge-thread.mjs);
    //   - approval:该会话的敏感动作(工作区外写入 / 命令执行)先在面板里问一句,
    //     8 秒没人答就交给官方链路(见 lib/bridge-approval.mjs)。
    if (result.ok === true && typeof result.sessionId === 'string' && result.sessionId !== '') {
      const ok = bridgeThread.watch(result.sessionId);
      if (!ok) console.warn('[code-server] 编辑器桥:会话订阅未建立(宿主没有 sessionController.follow)');
      bridgeApproval.intercept(result.agent, result.sessionId);
    }
    // `agent` 是内部句柄,不能进 JSON。
   ...
   * 面板对授权请求的决策(0.3.22)。
   *
   * **这是命名空间里唯一的非只读路由**,约束写死在实现里:
   *   - 只能回答**本 host 发起、仍未决**的请求(单次使用,用后即废);
   *   - outcome 只接受 `allowed-once` / `rejected`;
   *   - 不接受任何自由文本、路径或命令参数 —— 它只能"回答问题",不能"发起动作"。
   */
  async function handleBridgeApprove(request) {
    const denied = bridgeRejection(request);
    if (denied !== null) return denied;
    let body;
    try {
  ...
  /** 面板产物(thread.js / thread.css)给对话框注入用。 */
  async function handleAskBundle() {
    const bundle = askBundle();
    if (bundle === null) {
      return jsonResponse({ ok: false, error: '面板产物缺失:请在插件源码里跑 pnpm run build:webview,或重装插件' }, 503);
    }
    return jsonResponse({ ok: true, rev: askRevision(), ...bundle });
  }
  ...
  const disposers = [    { path: `${API_BASE}/status`, methods: ['GET'], fetch: handleStatus },
    ...
    { path: `${API_BASE}/ask/bundle`, methods: ['GET'], fetch: handleAskBundle },
