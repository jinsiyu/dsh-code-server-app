...
export async function apply(ctx, config) {
  ...
  /** 面板最近一次声明 watch 的时间/id:据此判断"面板是不是正在看"(关掉就不拦截授权)。 */
  const bridgeWatch = { ids: [], at: 0 };
  const bridgeApproval = createApprovalInterceptor({
    board: bridgeApprovalBoard,
    holdMs: DEFAULT_HOLD_MS,
    hasPanel: () => bridgeWatch.ids.length > 0 && Date.now() - bridgeWatch.at < 5000,
  ...
  async function handleBridgeSync(request) {
    ...
    bridgeThread.sync(bridgeWatch.ids);
    ...
    return jsonResponse({
      ...
      thread: bridgeThread.supported()
        ? bridgeThread.snapshot(bridgeWatch.ids)
  ...
  async function handleBridgeApprove(request) {
    ...
    if (id === '') return jsonResponse({ ok: false, error: '需要 id 字段' }, 400);
    if (!PANEL_OUTCOMES.includes(outcome)) {
      return jsonResponse({ ok: false, error: `outcome 只能取 ${PANEL_OUTCOMES.join(' | ')}` }, 400);
    }
    if (bridgeApprovalBoard.get(id) === null) {
      return jsonResponse({ ok: false, error: '未知或已处理的授权请求' }, 409);
    }
    const ok = bridgeApprovalBoard.answer(id, outcome);
    return jsonResponse({ ok, outcome: ok ? outcome : null }, ok ? 200 : 409);
  }
