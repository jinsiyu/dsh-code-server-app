 ...
 *
 * 方向:DSH → 编辑器。编辑器里的「问 DSH」面板/对话框要像 DSH 对话那样显示内容,数据来自
...
import { splitEditorPrompt } from './bridge-session.mjs';
...
export function applyEvent(state, event) {
  ...
  if (event.type === 'user/message') {
    ...
    // **消息来源**(0.3.43):别的工具注入的上下文也被 DSH 记成 user/message,但它们的 source
    // 不是 `{kind:'user'}`(官方界面按"上下文注入"折叠显示)。面板照做:只有 kind==='user'
    // 才是用户气泡,其余的渲染成折叠的「上下文注入」。
    const source = data.source ?? data.message?.source ?? null;
    const sourceKind = source !== null && typeof source === 'object' && typeof source.kind === 'string'
      ? source.kind
      : 'user';
    if (sourceKind !== 'user') {
      push(state, { role: 'user', text, sourceKind, context: '' });
      return true;
    }
    // 桥自己拼的消息(位置行 + 选区代码块 + 用户原话)拆开:上下文折叠显示,气泡只留原话。
    // 认不出来(用户在 DSH 界面里自己敲的)→ 整段当原话。
    const { context, question } = splitEditorPrompt(text);
    if (context === '') push(state, { role: 'user', text });
    else push(state, { role: 'user', text: question, context });
    return true;
  }
 ...
 * 任何可见变化都让 `rev` +1:对话框靠它判断"要不要拉全量状态"(每 900ms 只问一个数字)。
 */
export function consumeFrame(state, frame) {
  ...
  let changed = false;
  if (frame.type === 'assistant-stream') changed = applyStreamFrame(state, frame.frame);
  else if (frame.type === 'event') changed = applyEvent(state, frame.event);
  // 兼容:调用方直接喂会话事件(测试与降级路径)
  else if (typeof frame.type === 'string' && frame.data !== undefined) changed = applyEvent(state, frame);
...
export function threadSnapshot(state) {
  return {
    ...
    /** 修订号:变了才需要重新拉全量(对话框的轮询靠它省流量)。 */
    rev: Number.isSafeInteger(state.rev) ? state.rev : 0,
    entries: state.entries.map((entry) => ({
      role: entry.role,
      text: entry.text ?? null,
      // 思考过程(0.3.23):面板渲染成官方那样的折叠行,默认收起。
      thinking: entry.thinking ?? null,
      // 注入的上下文(0.3.24):桥自己拼的位置行 + 选区代码块,面板折叠显示(0.3.23 之前是平铺)。
      ...
      // 消息来源(0.3.43):'' / null = 用户自己说的;其它值(plugin 等)= 上下文注入,面板折叠显示。
...
export function createThreadRegistry(deps) {
  ...
  return {
    ...
    /**
     * 被观看会话的修订号(0.3.24):对话框每 900ms 只问这一个数字,变了才拉全量状态。
     * 没 watch 到就返回 0。
     */
    rev(sessionIds) {
      const wanted = Array.isArray(sessionIds) ? sessionIds : [...watched.keys()];
      for (const id of wanted) {
        const entry = watched.get(id);
        if (entry !== undefined) return entry.state.rev;
      }
      return 0;
    },
