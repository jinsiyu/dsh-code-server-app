...
export function createAssetMirror({ getTarget, tree, productPath, log = () => {} }) {
  ...
  function snapshot() {
    // enabled 是 host 侧的急停开关:客户端只在它非 false 时才把文档搬到镜像
    return { enabled: true, base: ASSET_BASE, document: ASSET_DOCUMENT, total: all.length, ...stats };
  }

  return { register, snapshot, base: ASSET_BASE, document: ASSET_DOCUMENT };
}
