 ...
 *  `vscode-remote-resource`(**扩展资源端点**)有**两种形态**,必须都覆盖:
 *   - 不带前缀(`/vscode-remote-resource`):语言包的 NLS bundle 走这个(实测);
 *   - 带 productPath 前缀(`/<productPath>/vscode-remote-resource`):工作台的**文件服务**走这个 ——
 *     主题的 include 链、TextMate 语法、seti 图标主题全靠它读。
 *  漏掉带前缀那一种的后果很隐蔽:请求在 **host 层就 404**(根本到不了 IDE),桌面端表现为
 *  "主题停在默认、代码无着色、图标全灰",而 launcher 的 request.log 里连这些请求都看不到
 *  (2026-09-11 实测,排查绕了很久)。 */
const SINGLETONS = [
  '/manifest.json',
  '/favicon.ico',
  '/favicon.svg',
...
function walkFiles(root, prefix, out, depth = 0) {
  if (depth > 12) return; // 防链接环
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) { walkFiles(full, url, out, depth + 1); continue; }
    if (entry.isFile()) { out.add(url); continue; }
    // **符号链接 / Windows junction 也要跟**:精简树的 lib/vscode/node_modules 全是 junction,
    // 语法引擎(vscode-textmate / vscode-oniguruma,含 onig.wasm)就在里面 —— 不跟随的话
    // 这些文件一条路由都没有,工作台加载不到语法引擎 ⇒ 代码完全没有着色(2026-09-11 实测)。
    if (entry.isSymbolicLink()) {
      let stat;
      try { stat = fs.statSync(full); } catch { continue; }
      if (stat.isDirectory()) walkFiles(full, url, out, depth + 1);
      else if (stat.isFile()) out.add(url);
    }
  }
}
...
export function enumerateAssetPaths(tree, productPath) {
  ...
  // 工作台的文件服务形态(读扩展资源:主题 include / 语法 / 图标主题)—— 与不带前缀的那种都要有
  urls.add(`/${productPath}/vscode-remote-resource`);
  // **以及"文档路径 + 产品路径"那一种**:工作台把端点拼成 `<文档路径>/<productPath>/vscode-remote-resource`,
  // 而我们的文档是 `…/asset/index.html`,于是路径里多出一段 `index.html/`。
  // web(serve: dsh)的 /code-server 是**前缀路由**,天然吃得下;镜像用的是**精确路由**,
  // 不显式注册这一形态 ⇒ 文件服务全线 404 ⇒ "主题停在默认、代码无着色、图标全灰"
  // (2026-09-11 靠页面信标抓出真实请求 URL 才定位到)。
  urls.add(`${ASSET_DOCUMENT.slice(ASSET_BASE.length)}/${productPath}/vscode-remote-resource`);
  ...
  // 树自带的运行时依赖(工作台按 `static/node_modules/…` 取:语法引擎 vscode-textmate /
  // vscode-oniguruma(onig.wasm)、markdown/图标等;精简树里是 junction,walkFiles 会跟随)
  walkFiles(path.join(tree, 'lib', 'vscode', 'node_modules'), `${staticRoot}/node_modules`, urls);
...
export function createAssetMirror({ getTarget, tree, productPath, log = () => {} }) {
  ...
  function register(connection) {
    const disposers = [];
    // 文档段(`/index.html/`):工作台按 location.pathname 拼扩展资源端点时会带上它,
    // 注册时要保留(否则 host 层 404),但**转发给 IDE 前必须去掉**(IDE 只认 /<productPath>/…)。
    const docSegment = `${ASSET_DOCUMENT.slice(ASSET_BASE.length)}/`;
    for (const urlPath of registrable) {
      const routePath = `${ASSET_BASE}${urlPath}`;
      const upstreamPath = urlPath.startsWith(docSegment) ? `/${urlPath.slice(docSegment.length)}` : urlPath;
      disposers.push(connection.fetch.register({
        path: routePath,
        methods: ['GET', 'HEAD'],
        requestBody: 'buffered',
        fetch: (request) => forward(request, upstreamPath),
      }));
    }
