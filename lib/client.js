 ...
 *
 * DSH 支持范围(2026-09-22 收敛;判据一律是**能力是否被声明**,不做版本号比较):
 *   · rc 线    0.1.5-rc.x(最新 0.1.5-rc.3 = npm latest/next)
 *              旧座位 `settings.plugin.item` + 会话列表快照上的 `current`
 *   · alpha 线 ≥ 0.1.6-alpha.2(最新 0.1.7-alpha.1)
 *              新座位 `plugins.bundle.config` + 会话作用域标准 prop `sessionId`
...
let React = require('react')
    ...
    //   · `plugins.bundle.config`(alpha 线 ≥ 0.1.6-alpha.2,含最新 0.1.7-alpha.1):插件页按**包名**派发的配置区,页面自带
    ...
    function csSettingsPage(props) {
      ...
      var loaded = {
        ...
        fim: value.fim === true,
        fimDebounceMs: normalizeFimDebounce(value.fimDebounceMs),
        fimMultiline: value.fimMultiline !== false,
        fimDisableGlobs: typeof value.fimDisableGlobs === 'string' ? value.fimDisableGlobs : '',
      }
      ...
      var overriddenFim = user !== undefined && user !== null && Object.prototype.hasOwnProperty.call(user, 'fim')
      var overriddenFimDebounce = user !== undefined && user !== null && Object.prototype.hasOwnProperty.call(user, 'fimDebounceMs')
      var overriddenFimMultiline = user !== undefined && user !== null && Object.prototype.hasOwnProperty.call(user, 'fimMultiline')
      var overriddenFimGlobs = user !== undefined && user !== null && Object.prototype.hasOwnProperty.call(user, 'fimDisableGlobs')
      var dirty = draft !== null && (draft.keepResident !== loaded.keepResident
        ...
        || draft.fim !== loaded.fim
        || normalizeFimDebounce(draft.fimDebounceMs) !== normalizeFimDebounce(loaded.fimDebounceMs)
        || draft.fimMultiline !== loaded.fimMultiline
        || draft.fimDisableGlobs !== loaded.fimDisableGlobs)
      ...
      var fields = [
        ...
        React.createElement('div', { className: 'dshcs-field' },
          ...
          React.createElement(csCheck, {
            checked: draft !== null ? draft.fim : loaded.fim,
            disabled: snapshot.writable !== true,
            onChange: function (v) { setDraft(function (prev) { return Object.assign({}, prev !== null ? prev : loaded, { fim: v === true }) }); setFailed(false) },
        ...
        React.createElement('div', { className: 'dshcs-field' },
          ...
          React.createElement(csNumber, {
            value: draft !== null ? draft.fimDebounceMs : loaded.fimDebounceMs,
            min: 100, max: 3000, step: 50, disabled: snapshot.writable !== true,
            onChange: function (v) { setDraft(function (prev) { return Object.assign({}, prev !== null ? prev : loaded, { fimDebounceMs: v }) }); setFailed(false) },
          }),
          React.createElement('div', { className: 'dshcs-hint', style: { marginTop: 4 } },
        ...
        React.createElement('div', { className: 'dshcs-field' },
          ...
          React.createElement(csCheck, {
            checked: draft !== null ? draft.fimMultiline : loaded.fimMultiline,
            disabled: snapshot.writable !== true,
            onChange: function (v) { setDraft(function (prev) { return Object.assign({}, prev !== null ? prev : loaded, { fimMultiline: v === true }) }); setFailed(false) },
        ...
        React.createElement('div', { className: 'dshcs-field' },
          ...
          React.createElement(csTextarea, {
            value: draft !== null ? draft.fimDisableGlobs : loaded.fimDisableGlobs,
            disabled: snapshot.writable !== true,
            placeholder: '例如 *.md;**/dist/**;vendor/**',
            onChange: function (v) { setDraft(function (prev) { return Object.assign({}, prev !== null ? prev : loaded, { fimDisableGlobs: v }) }); setFailed(false) },
          }),
          React.createElement('div', { className: 'dshcs-hint', style: { marginTop: 4 } },
...
`
    ...
    function internalApply(ctx) {
      ...
      // ① 插件页的配置区(alpha 线 ≥ 0.1.6-alpha.2,含最新 0.1.7-alpha.1):`settings.plugin.item` 退役,上游把插件配置搬到
      ...
      // ② 设置页里的插件卡片(rc 线 ≤ 0.1.5-rc.3,最新 rc 仍在用;也是没有右侧栏服务的旧 DSH 里唯一的提示出口)。
