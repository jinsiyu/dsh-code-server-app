...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    let ReactDOM = null
    try { ReactDOM = require('react-dom') } catch (e) { /* react-dom 不可用时降级为普通渲染 */ }
    ...
    var COMPOSER_PAD = 10 // composer 上方留白的额外余量
    ...
    function Window(props) {
      ...
      if (ReactDOM !== null && typeof ReactDOM.createPortal === 'function') {
    ...
    function csSettingsPage(props) {
      try {
        ...
        if (scope === undefined) {
          ...
          return React.createElement('div', null, 'scope missing')
        }
        if (snapshot === null || snapshot === undefined || snapshot.status !== 'ready') {
          // 尚未就绪:显示占位,不吞掉卡片(visible debugging)
          return React.createElement('div', { 'data-dshcs-pending': 'true', className: 'dshcs-cardName' }, 'Code Server: 设置尚未就绪(' + (snapshot !== null && snapshot !== undefined ? snapshot.status : 'no-snapshot') + ')')
        }
      ...
      return React.createElement(csCard, {
        ...
        return React.createElement('div', null, 'Code Server 卡片错误')
      }
    }
