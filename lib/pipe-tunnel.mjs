  ...
  async function fetch(request) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        ...
        // 关键:Node 的 ClientRequest 只在第一次 write 或 end 时才把 header 发出去。
        // 隧道的请求体在服务端回 101 之前一直静默(客户端要拿到握手才会发帧),
        // 不显式 flush 就是死锁:服务端等 body → 客户端等 101 → 请求根本没发出去
        // (实测:host 挂到 carrier 超时回 408,子进程 tunnel.log 永不出现)。
