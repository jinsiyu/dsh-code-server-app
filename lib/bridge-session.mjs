 ...
 *     —— 排到下一轮,不打扰当前轮。
 *
 * 读法走官方 API `ctx.settings.get(ns)`(宿主侧 `@deepseek-ai/dsh-settings` 的契约:
 * "Read one registered namespace's resolved value")。读不到(命名空间没注册 / 极简组合 / 老版本 DSH)
 * 一律回默认 `'queue'`,并且**逐项特性探测** `agent.steer` 是否存在 —— 探测失败绝不抛。
 *
 * @param {object} ctx cordis 上下文
 * @returns {'queue'|'steer'}
 */
export function pickBusyEnter(ctx) {
  ...
  if (settings === undefined || settings === null || typeof settings.get !== 'function') return 'queue';
  try {
    const section = settings.get('ui-conversation');
