...
export function createApprovalInterceptor(deps) {
  ...
  /** 本地生成的请求 id 序号(审批事件本身没有 id;见 handle 里的注释)。 */
  let localSeq = 0;
  ...
  async function waitForDecision(decision) {
    ...
    let loggedStillWaiting = false;
    for (;;) {
      const remaining = deadline - Date.now();
      if (remaining <= 0) {
        log('等待期间没人作答:窗口到点,交回 DSH 界面');
        return undefined;
      }
      ...
      if (!hasPanel()) {
        log('等待期间判据翻成 false(不再有人看):立刻交回 DSH 界面 —— 若卡片因此跑到 DSH 侧,这就是原因');
        return undefined;
      }
      if (!loggedStillWaiting) {
        loggedStillWaiting = true;
        log('已接住授权,正在等面板/对话框作答(此后再翻 false 会立刻放手)');
      }
    }
  }
  ...
  async function handle(request, next) {
    try {
      log('收到授权请求:' + String(request && request.toolName) + ' id=' + String(request && request.id) + ' hasPanel=' + hasPanel());
      // 面板没在看:完全不拦截,保持今天的行为。
      if (!hasPanel()) {
        log('没人看会话(hasPanel=false):不拦截,直接交给官方链路');
        return await next();
      }
      // **审批事件没有 `id` 字段**(0.3.39 的实测:日志里 `收到授权请求:pwsh id=undefined`)。
      // 事件形状是 `{agent, toolName, callId?, reason?, signal?}` —— 我们原来要求 id 是字符串,
      // 否则**静默 next()** 交回官方卡片:这就是"面板里的卡片从来不出现"的最后一块拼图。
      // id 只是我们自己用来把"面板上的那张卡"和"这次请求"对上号,所以本地生成即可;
      // `board.answer(id, outcome)` 结算的是我们自己的 decision promise,DSH 侧不需要知道它。
      const hasId = typeof request?.id === 'string' && request.id !== '';
      const id = hasId ? request.id : 'local-' + String(++localSeq);
      ...
      log('decision -> ' + JSON.stringify(outcome === undefined ? null : outcome) + ' (typeof=' + typeof outcome + ')');
