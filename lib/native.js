...
//   平台专属条目还带一个 `targets` 白名单(打包期由 scripts/repack-platforms.json 推导):
//   只属于别的平台的模块(如 Linux 上的 @vscode/windows-registry)**不进本平台的清单** ——
//   它对应的 `-linux-x64` 子包永远不会存在,报成「缺包」只会误导环境检测卡片。
//
// 本模块只做四件事:
