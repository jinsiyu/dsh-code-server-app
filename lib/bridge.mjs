 ...
 * ## 安全不变量(改这个文件之前先读这几条)
 *
 * 1. **`BRIDGE_BASE`(`/code-server-bridge`)命名空间**是**四条只读路由 + 一条有界的模型调用**:
 *    `/health`(无鉴权探活)、`/sync`(上报编辑器状态 + 取事件)、`/old`(取写前原文快照)、
 *    `/event`(上报意图,如"请宿主打开对话框"),以及 **`/complete`**(0.3.61,**实验性 FIM 补全**):
 *    它把光标前后的两段文本交给模型换回一段补全文本 —— **仍然不写文件、不执行命令**,
 *    而且默认关(设置里开启前一律 403),有速率/并发/长度/超时上限(见 lib/index.js 的
 *    handleBridgeComplete 与 lib/fim-adapter.mjs)。这里**不允许**出现写文件、改文档、执行命令、
 *    拉起进程的路由 —— `bridge.json` 里那个令牌对本机同用户进程可读,爆炸半径必须封在
 *    "泄露编辑器里的信息 + 替用户花一点补全 token",绝不能变成任意文件写 / 任意命令执行。
 *    提问与授权答复不走这里:它们在 DSH 同源的 `/api/code-server/ask/*` 上,调用方是 DSH 页面里的
 *    插件客户端(吃 DSH 自己的 cookie/Origin 校验)。
 * 2. **授权答复的四条约束**(在 `/api/code-server/ask/approve` 上,缺一条就等于开了任意命令执行的后门,
 *    不许放宽):
 *    (a) 它只能**回答**已经存在的授权请求,不能发起任何动作 —— 请求体只有 `{id, outcome}`,
 *        不接受自由文本 / 路径 / 命令参数;
 *    (b) `id` 必须是本进程自己发起、且**仍未决**的请求(用后即废,单次使用);
 *    (c) `outcome` 只接受 `allowed-once` / `rejected` 两个白名单值,没有"永久允许";
 *    (d) 对话框没在看 / 窗口超时(默认 5 分钟)→ **交回官方链路**,绝不自动放行(DSH 的
 *        `approval/request` 本身 fail closed,这里只能把"没人答"保持成"没人答")。
 * 3. **带 `Origin` 的请求一律 403。** 浏览器发起的请求必带 Origin,扩展宿主进程不带。
 *    这条只排除浏览器,不误伤扩展(也顺带挡住 CSRF / DNS rebinding 这类"用你的浏览器打本机端口")。
 * 4. **路径收敛在编辑器当前工作区**(由扩展侧 `workspace.getWorkspaceFolder` 判定)。
 * 5. **有界。** 事件缓冲 64 条、请求/响应体上限见下方常量;对话流每会话 ≤120 条、单条正文 ≤8000 字符、
 *    同时 watch ≤4 个会话,待决授权 ≤4 条(见 lib/bridge-thread.mjs / lib/bridge-approval.mjs)。
 */
