 ...
 * ## 安全不变量(改这个文件之前先读这四条)
 *
 * 1. **`BRIDGE_BASE`(`/code-server-bridge`)命名空间只读**,只有一个写例外:`POST /approve`
 *    (0.3.22,见下)。其余一律不允许出现写文件、改文档、执行命令、拉起进程的路由 ——
 *    `bridge.json` 里那个令牌对本机同用户进程可读,爆炸半径必须封在"泄露编辑器里的信息",
 *    绝不能变成任意文件写 / 任意命令执行。
 ...
 *    (d) 没有面板在看 / 窗口超时(默认 8s)→ **交回官方链路**,绝不自动放行(DSH 的
