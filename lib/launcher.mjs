 ...
 *
 *  上报目标要按"文档是谁给的"选:
 *   - web(serve: dsh):文档就来自 launcher 自己,`/__dshcs/report` 同源可达 → 落 page-errors.log;
 *   - desktop:文档来自 host 的资产镜像(dsh-app://…/api/code-server/asset/index.html),
 *     `/__dshcs/report` 会落在 **host** 上而到不了 launcher(0.3.1 起这个文件因此一直是空的),
 *     改报 host 的 `/api/code-server/diag`(落 client-diag.log)。
 *  另外把页面 console.error/warn 与**资源加载失败**(捕获阶段,<link>/<script>/<img> 不冒泡)也报回来 —— 
 *  工作台自己的报错(比如"主题解析不了")只出现在 console 里,不修这条我们就永远看不到客户端真相。 */
function diagnosticsScript(nonce) {
  const attr = nonce === null ? '' : ` nonce="${nonce}"`;
  return `<script${attr}>
(function(){
  var REPORT = location.pathname.indexOf('/api/code-server/asset/') === 0
    ? '/api/code-server/diag'
    : '/__dshcs/report';
  var budget = 60;
  // 通道选择(按可靠性排序,全都试一遍,谁先成功算谁):
  //  ① **图片信标**:打到镜像里必定注册的极小文件(robots.txt),载荷放在查询串 → launcher 的
  //     request.log 里直接可读。它不依赖客户端 bundle(中继)、不依赖 host 的 /api 通道、也不受
  //     sendBeacon 的 scheme 限制限制 —— 桌面端两次重启"一条诊断都没到"之后加的这条。
  //  ② postMessage → 父窗口中继(src/pipe-relay.js)→ host /api/code-server/diag。
  //  ③ sendBeacon / fetch 到 REPORT(web 模式下文档就在 launcher 源上,这条最直接)。
  var MIRROR_DOC = location.pathname.indexOf('/api/code-server/asset/') === 0;
  var BEACON = MIRROR_DOC ? '/api/code-server/asset/robots.txt' : '/_static/src/browser/robots.txt';
  function send(entry){
    if (budget-- <= 0) return;
    try {
      var payload = encodeURIComponent(JSON.stringify(entry)).slice(0, 1500);
      var img = new Image();
      img.src = BEACON + '?d=' + payload;
    } catch(e){ /* 信标失败不影响后续通道 */ }
    if (MIRROR_DOC) {
      try {
        if (window.parent && window.parent !== window) {
          window.parent.postMessage({ __dshcs: 1, kind: 'diag', entry: entry }, '*');
          return;
        }
      } catch(e){ /* 落到下面的 fetch */ }
    }
    var body;
    try { body = JSON.stringify(entry); } catch(e){ return; }
    // dsh-app:// 这类自定义 scheme 下 sendBeacon 会**抛 TypeError**(只接受 http(s)),
    // 必须单独兜住:它和 fetch 共用一个 try 时,一抛就连回退也走不到(实测白改一场)。
    try { if (navigator.sendBeacon && navigator.sendBeacon(REPORT, body)) return; } catch(e){}
    try { fetch(REPORT, { method: 'POST', body: body, keepalive: true }); } catch(e){}
  }
  // 扩展资源读取的 URL 医治:自定义 scheme(dsh-app://)下 **Service Worker 注册不了**
  // ("The URL protocol of the current origin is not supported"),于是 VS Code 读 vscode-remote://
  // 资源时退化成 http://<authority>:80/<path> —— 一个根本不可达的地址(TCP 都没有),
  // 报错长这样:无法加载 vscode-remote://app/…/theme-defaults/themes/2026-dark.json: Failed to fetch。
  // 路径本身是对的(已经带镜像前缀),把 scheme/host 换回**文档自己的相对路径**即可;
  // 顺便把 XHR 也治上(不同版本走不同通道)。注意:这段脚本整体是模板字符串,注释里不能用反引号。
  var HEAL = /^https?:\/\/[^/]+(\/api\/code-server\/asset\/)/;
  function healUrl(u){
    try {
      var m = HEAL.exec(String(u));
      return m === null ? u : m[1];
    } catch(e){ return u; }
  }
  // 包装 fetch:① 就医治不可达的扩展资源 URL;② 把工作台**自己的**资源读取连 URL 带状态记下来
  // (主题 include / TextMate 语法 / 图标主题都走 vscode-remote-resource,是"文件服务通不通"的直接证据)。
  (function(){
    var orig = window.fetch;
    if (typeof orig !== 'function') return;
    var watch = /vscode-remote-resource|\.tmLanguage|\.tmGrammar|theme-defaults|seti/i;
    window.fetch = function(input, init){
      var url = '';
      var target = input;
      try {
        url = typeof input === 'string' ? input : (input && input.url) || '';
        var fixed = healUrl(url);
        if (fixed !== url) {
          if (typeof input === 'string') target = fixed;
          else if (typeof Request === 'function') target = new Request(fixed, input);
          send({ kind:'url-healed', from:String(url).slice(0,200), to:String(fixed).slice(0,120) });
        }
      } catch(e){ target = input; }
      var p = orig.call(this, target, init);
      try {
        if (watch.test(String(url))) {
          p.then(function(r){ send({ kind:'fetch-log', url:String(url).slice(0,240), status:r.status }); },
                 function(e){ send({ kind:'fetch-error', url:String(url).slice(0,240), message:String(e && e.message || e) }); });
        }
      } catch(e){ /* 忽略 */ }
      return p;
    };
    ...
    if (typeof xhrOpen === 'function') {
      window.XMLHttpRequest.prototype.open = function(method, requestUrl){
        try { arguments[1] = healUrl(requestUrl); } catch(e){ /* 原样 */ }
  ...
  window.addEventListener('error', function(e){
    ...
    if (t && t !== window && (t.src || t.href)) {
      send({ kind:'resource-error', tag:String(t.tagName||''), url:String(t.src||t.href||'').slice(0,300) });
      return;
    }
  ...
  ['error','warn'].forEach(function(level){
    ...
    console[level] = function(){
      try {
        ...
        send({ kind:'console-' + level, message:parts.join(' ').slice(0, 600), href:location.href });
      } catch(e){}
      return orig.apply(console, arguments);
    };
  });
  send({ kind:'boot', message:'workbench html loaded', factory: typeof globalThis.__DSH_WS_FACTORY__,
    href:location.href, pathname:location.pathname, report:REPORT, ua:navigator.userAgent });
  // 工作台会在 body 上写下真正生效的主题;3s 后连同几个计算样式一起报回来(排查配色用)
  setTimeout(function(){
    try {
      var cs = getComputedStyle(document.body);
      send({ kind:'theme', themeKind:document.body.dataset.vscodeThemeKind || '(none)',
        themeName:document.body.dataset.vscodeThemeName || '(none)',
        colorScheme:document.documentElement.style.colorScheme || '(none)',
        bodyBg:cs.backgroundColor, bodyColor:cs.color });
    } catch(e){ send({ kind:'theme-error', message:String(e && e.message || e) }); }
  }, 3000);
})();
</script>`;
}
...
function injectWebSocketFactory(text) {
  ...
  if (hits !== 1) {
    // 没有回退路径:客户端只能用 shim 产出的字节走隧道(IDE 不监听端口)。
    warnOnce('marker', `工作台 bundle 的注入标记命中 ${hits} 次(期望 1):跳过注入 —— 客户端将无法建立连接(无回退路径,多为 VS Code 版本变了)`);
    return null;
  }
  shimInjections += 1;
...
/** 请求日志(排查"客户端到底拉了什么、拿回什么"):只记方法/路径/状态,带 2MB 上限。
 *  典型用途:工作台的主题 include、TextMate 语法、图标主题都靠 `vscode-remote-resource` 读,
 *  颜色/图标不对时先看这里——是没请求,还是请求了但 4xx。 */
const REQUEST_LOG = process.env.DSHCS_REQUEST_LOG ?? null;
const REQUEST_LOG_MAX = 2 * 1024 * 1024;
let requestLogSize = 0;
try { if (REQUEST_LOG !== null) requestLogSize = statSync(REQUEST_LOG).size; } catch { /* 首次 */ }
let requestLogTrimmed = false;
function requestLog(line) {
  if (REQUEST_LOG === null) return;
  try {
    if (requestLogSize > REQUEST_LOG_MAX) {
      if (requestLogTrimmed) return;
      requestLogTrimmed = true;
      writeFileSync(REQUEST_LOG, `${new Date().toISOString()} [launcher] 日志超过 2MB,后续不再记录\n`);
      requestLogSize = 0;
      return;
    }
    const text = `${new Date().toISOString()} ${line}\n`;
    appendFileSync(REQUEST_LOG, text);
    requestLogSize += Buffer.byteLength(text, 'utf8');
  } catch { /* 日志失败不影响功能 */ }
}

const server = createHttpServer((req, res) => {
  const url = req.url ?? '/';
  // **只按路径匹配,别拿整串比**:客户端加载文档时永远带查询串(`?s=<pid>&folder=…`),
  // 以前这里用 `url === '/'` 判断 ⇒ HTML 改写整条被跳过 ⇒ `?v=` 缓存击穿与注入的诊断脚本
  // 在真实运行里**从未生效**(本地探针用裸 `/` 所以一切正常,长期掩盖了这个问题)。
  const urlPath = url.split('?')[0];
  const isDiagBeacon = url.includes('?d=');
  if (REQUEST_LOG !== null && urlPath !== '/healthz' && (isDiagBeacon || !urlPath.startsWith('/_static/'))) {
    // 截 2000 字:诊断信标把载荷放在查询串里(?d=…),截短了就只能看到"有请求"看不到内容
    res.on('finish', () => requestLog(`${req.method ?? 'GET'} ${url.slice(0, 2000)} -> ${res.statusCode}`));
  }
  if (urlPath === TUNNEL_ROUTE) {
    handleTunnel(req, res, url);
    return;
  }
  if (urlPath === PAGE_REPORT_ROUTE) {
    let body = '';
  ...
  if (NEEDS_SERVE_INJECTION && urlPath.endsWith(WORKBENCH_BUNDLE_SUFFIX)) {
