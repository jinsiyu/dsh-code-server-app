 ...
 *
 *  为什么不能把隧道字节直接喂给 vsServer:VS Code 会把连接的 **socket 句柄**通过 IPC
 *  `child.send(msg, handle)` 交给扩展宿主进程,句柄必须是真实 socket(net.Socket/命名管道)。
 *  用自造的 Duplex 会得到 `ERR_INVALID_HANDLE_TYPE: This handle type cannot be sent`
 *  → 扩展宿主反复退出重启 → 所有扩展(含内置的 dshcs-open-file)都不激活
 *  (实测症状:IDE 界面能看,但点击文件只写了信号文件、没人消费)。
 *
 *  所以隧道先桥接到一对**真实 socket**:本进程开一个唯一命名的 Windows 命名管道
 *  (不开 TCP 端口),自己连上去,隧道字节走这条管道;交给 handleUpgrade 的是管道服务端的
 *  真实 socket —— 句柄因此可以正常传给扩展宿主。
 */
function handleTunnel(req, res, url) {
  ...
  // 才把连接当纯字节流,否则它会套一层帧层、客户端解析不了(实测:连接秒断、无限重连)。
  ...
  let inChunks = 0;
  tunnelSeq += 1;
  const pipePath = `\\\\.\\pipe\\dshcs-tunnel-${process.pid}-${tunnelSeq}`;
  let bridge = null;
  let serverSide = null;

  const pipeServer = createNetServer({ pauseOnConnect: true });
  ...
  const finish = (reason) => {
    ...
    try { pipeServer.close(); } catch { /* ignore */ }
    try { bridge?.destroy(); } catch { /* ignore */ }
    try { serverSide?.destroy(); } catch { /* ignore */ }
  };

  pipeServer.on('error', (error) => {
    log(`隧道管道服务出错:${error && error.message ? error.message : error}`);
    finish('pipe server error');
  });

  pipeServer.on('connection', (socket) => {
    if (closed || serverSide !== null) { socket.destroy(); return; }
    serverSide = socket;
    // 与 loopback 路径逐字对齐:handleUpgrade 在升级期间 pause socket,调用方随后 resume。
    socket.pause();
  ...
  pipeServer.listen(pipePath, () => {
    bridge = connect(pipePath);
    bridge.on('error', (error) => {
      log(`隧道桥接出错:${error && error.message ? error.message : error}`);
      finish('bridge error');
    });
    bridge.on('end', () => finish('bridge ended'));
    // 服务端 → 隧道响应体(带背压:响应写不动时先暂停管道读取)
    bridge.on('data', (chunk) => {
      ...
      if (!res.write(chunk)) {
        bridge.pause();
        res.once('drain', () => bridge.resume());
      }
    });
  });

  // 隧道请求体 → 桥接客户端(再经命名管道到真实 socket)
  req.on('data', (chunk) => {
    ...
    if (bridge !== null) bridge.write(chunk);
  });
  req.on('end', () => { try { bridge?.end(); } catch { /* ignore */ } });
