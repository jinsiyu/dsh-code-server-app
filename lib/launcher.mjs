...
(function(){
  ...
  function send(entry){
    ...
    try{
      ...
      // 自定义 scheme(dsh-app://)下 sendBeacon 可能直接失败 → 返回 false 时退回 fetch
      if (navigator.sendBeacon && navigator.sendBeacon(REPORT, body)) return;
      fetch(REPORT, { method: 'POST', body: body, keepalive: true });
    }catch(e){}
  }
