...
(function(){
  var REPORT = location.pathname.indexOf('/api/code-server/asset/') === 0
    ...
    : '/__dshcs/report';
  var budget = 60;
  // 桌面端文档来自 host 资产镜像,而 iframe 直接 fetch /api 到不了 host(实测:改成报 diag 也一条没到)。
  // 因此走**已被验证可用**的通道:postMessage → 父窗口的中继(src/pipe-relay.js)→ host /api/code-server/diag。
  // web(serve: dsh)下文档就在 launcher 源上,照旧 fetch 自己的 /__dshcs/report。
  var VIA_PARENT = location.pathname.indexOf('/api/code-server/asset/') === 0;
  function send(entry){
    if (budget-- <= 0) return;
    if (VIA_PARENT) {
      try {
...
const server = createHttpServer((req, res) => {
  const url = req.url ?? '/';
  if (REQUEST_LOG !== null && !url.startsWith('/_static/') && url !== '/healthz') {
    res.on('finish', () => requestLog(`${req.method ?? 'GET'} ${url.slice(0, 300)} -> ${res.statusCode}`));
  }
