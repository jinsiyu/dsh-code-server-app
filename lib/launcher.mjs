 ...
 *  出方向(→ 客户端)的字节写进本 socket 的可读侧,入方向(客户端 →)写进响应的可写侧。
 *  实现成 Duplex 而不是裸对象:`ws` 会调用 socket.setTimeout/setNoDelay/destroy 等。 */
class TunnelSocket extends Duplex {
  ...
  _write(chunk, _encoding, callback) {
    try {
      if (this.writeToClient(chunk) === false) this.once('drain', callback);
      else callback();
    } catch (error) {
      callback(error);
    }
...
/** 隧道事件日志(阶段 1 验收用):host 侧读这个文件即可确认"WS 真的走了隧道"。 */
...
function handleTunnel(req, res, url) {
  ...
  let bodyBytes = 0;
  let sentBytes = 0;
  const socket = new TunnelSocket((chunk) => {
  ...
  req.on('data', (chunk) => { bodyBytes += chunk.length; socket.feed(chunk); });
  ...
  try {
    vsServer.handleUpgrade(upgrade, socket);
  } catch (error) {
