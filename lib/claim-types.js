 ...
 *
 * 本模块是 host(`lib/index.js` 的 Config 默认值与工具实现)的**事实来源**,随 npm 包一起发布
 * (见 package.json 的 files),避免两边默认值漂移。
 * 客户端半部 `lib/client.js` 里有一份**内联副本**(经典脚本拿不到 host 模块,见那个文件的
 * "认领类型"段),两份逐字等价由 scripts/test-client-entry.mjs 的 E2 守卫钉住 —— 改这里必须同步改那边。
...
export const PREVIEW_FRIENDLY_EXTENSIONS = [
...
export const DEFAULT_CLAIM_EXTENSIONS = ['*']
